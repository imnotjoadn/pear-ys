...
function Comparisons(props: Props) {
    ...
    const comparisons = useSelector<RootState, { [key: string]: Pair; }>((state: RootState) => state.firestore.ordered.list);
    ...
    return (
        <React.Fragment>
            <ul>
                {comparisons && Object.keys(comparisons).map((key:string) =>
                    <li key={key}>{comparisons[key].title}</li>)}
            </ul>
        </React.Fragment>
    );
}
